...
def _safe_screenshot_path(requested: str | None) -> Path:
    fallback = Path.home() / "Desktop" / "sara_screenshot.png"
    if not requested:
        return fallback
    try:
...
def _type(text: str, interval: float = 0.01) -> str:
    _require_pyautogui()
    time.sleep(0.05)
...
def _move(x: int, y: int, duration: float = 0.05) -> str:
    _require_pyautogui()
    pyautogui.moveTo(x, y, duration=duration)
    return f"Mouse → ({x}, {y})"


def _drag(x1: int, y1: int, x2: int, y2: int, duration: float = 0.12) -> str:
    _require_pyautogui()
...
def _screen_find(description: str) -> tuple[int, int] | None:
    ...
    try:
        ...
        if match:
            ...
            # Scale coordinates to logical screen coordinates expected by PyAutoGUI
