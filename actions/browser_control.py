...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".sara_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".sara_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using SARA profile")
                sara = str(Path.home() / ".sara_profiles" / "firefox_sara")
                Path(sara).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(sara, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".sara_profiles" / "safari")
        ...
        sara_profile = str(Path.home() / ".sara_profiles" / self.browser_name)
        Path(sara_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with SARA profile: {sara_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(sara_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            print(f"[Browser] ✅ Launched [{label}] with SARA profile")
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "sara_screenshot.png")
