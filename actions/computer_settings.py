...
def get_volume() -> str:
    """Return current system volume level as a string."""
    if _OS == "Windows":
        try:
            import math
            from ctypes import cast, POINTER
            ...
            pct       = round(10 ** (db / 20) * 100) if db > -65 else 0
            return f"Volume is at {pct}%."
        except Exception as e:
        ...
        try:
            r = subprocess.run(["osascript", "-e",
                ...
                capture_output=True, text=True)
            return f"Volume is at {r.stdout.strip()}%."
        except Exception as e:
            return f"Could not read volume: {e}"
...
def get_battery() -> str:
    ...
    try:
        ...
        if batt is None:
            return "No battery detected (desktop or API unavailable)."
        ...
        time_str = f", {mins // 60}h {mins % 60}m remaining" if mins else ""
        return f"Battery is at {batt.percent:.0f}% ({status}{time_str})."
    except ImportError:
        return "psutil not installed — cannot read battery."
    except Exception as e:
...
def open_camera():
    if _OS == "Windows":
        ...
        for cam in [["cheese"], ["guvcview"], ["mpv", "av://v4l2:/dev/video0"]]:
            if subprocess.run(["which", cam[0]], capture_output=True).returncode == 0:
                subprocess.Popen(cam)
                return


def open_notification_center():
    if _OS == "Windows":
        pyautogui.hotkey("win", "n")   # Windows 11 notification panel
    elif _OS == "Darwin":
        # Notification Centre toggle on macOS
        import ctypes
        # Use keyboard shortcut
        pass
    # No universal Linux equivalent
...
ACTION_MAP: dict[str, callable] = {
    ...
    "notification_center":   open_notification_center,
    "new_desktop":           virtual_desktop_new,
    "virtual_desktop_new":   virtual_desktop_new,
    "next_desktop":          virtual_desktop_next,
    "virtual_desktop_next":  virtual_desktop_next,
    "prev_desktop":          virtual_desktop_prev,
...
def _screen_find(description: str) -> tuple[int, int] | None:
    ...
    if not api_key:
        print("[Settings] ⚠️ No API key for screen_find")
        return None
    ...
    try:
        ...
        prompt = (
            f"This is a screenshot of a {img_w}×{img_h} pixel screen. "
        ...
        if match:
            coords_x = int(match.group(1))
            coords_y = int(match.group(2))
            final_x = int(coords_x * w / img_w)
            final_y = int(coords_y * h / img_h)
            return final_x, final_y

    except Exception as e:
        print(f"[Settings] ⚠️ _screen_find failed: {e}")
...
def _detect_action(description: str) -> dict:
    ...
    available = ", ".join(sorted(ACTION_MAP.keys())) + \
                ", volume_set, type_text, press_key, reload_n, click, screen_click, click_button"
...
- For click, screen_click, click_button: value is the description of the button/element to click on the screen/desktop (e.g. "the start button", "the send button", "WhatsApp icon").
...
def computer_settings(
    ...
    if action in ("click", "click_element", "screen_click", "click_button"):
        desc = str(value or params.get("text", "") or description).strip()
        if not desc:
            return "No element description provided to click."
