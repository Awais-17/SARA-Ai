"""
file_processor.py — SARA Universal File Processor
        ...
        if isinstance(contents, list):
            ...
            for item in contents:
                if isinstance(item, dict) and "data" in item and "mime_type" in item:
                    part = types.Part.from_bytes(data=item["data"], mime_type=item["mime_type"])
                    new_contents.append(part)
                else:
                    new_contents.append(item)
            contents = new_contents
        elif isinstance(contents, dict) and "data" in contents and "mime_type" in contents:
            contents = types.Part.from_bytes(data=contents["data"], mime_type=contents["mime_type"])
...
def _gemini_client():
    return _GenaiWrapper("gemini-2.5-flash")
